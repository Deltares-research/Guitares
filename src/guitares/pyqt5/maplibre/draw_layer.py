...
class DrawLayer(Layer):
    def __init__(
        ...
        super().__init__(maplibre, id, map_id)
    ...
    def set_mode(self, mode):
        ...
        if mode == "active":
            ...
            self.active = False
            self.visible = False
        self.maplibre.runjs("./js/draw_layer.js", "setLayerMode", arglist=[self.map_id, mode])

    def activate(self):
        """Activate the draw layer so it can be edited by the user."""
        self.active = True
        self.maplibre.runjs("./js/draw_layer.js", "setLayerMode", arglist=[self.map_id, "active"])

    def deactivate(self):
        """Deactivate the draw layer so it can not be edited by the user."""
        self.active = False
        self.maplibre.runjs("./js/draw_layer.js", "setLayerMode", arglist=[self.map_id, "inactive"])
    ...
    # def hide(self):
    #     self.visible = False
    #     self.maplibre.runjs("./js/draw_layer.js", "setLayerMode", arglist=[self.map_id, "invisible"])
    ...
    def add_feature(self, gdf):
        ...
        for index, row in gdf.to_crs(4326).iterrows():
            ...
            self.maplibre.runjs("./js/draw_layer.js", "addFeature", arglist=[gdf, self.map_id])
    ...
    def add_rectangle(self, x0, y0, lenx, leny, rotation):
        ...
        project = pyproj.Transformer.from_crs(
            self.maplibre.crs, pyproj.CRS(4326), always_xy=True
        ).transform
    ...
    def draw(self):
        ...
        if self.shape == "polygon":
            self.maplibre.runjs("./js/draw_layer.js", "drawPolygon", arglist=[self.map_id])
        elif self.shape == "polyline":
            self.maplibre.runjs("./js/draw_layer.js", "drawPolyline", arglist=[self.map_id])
        elif self.shape == "rectangle":
            self.maplibre.runjs("./js/draw_layer.js", "drawRectangle", arglist=[self.map_id])
    ...
    def feature_drawn(self, feature_collection, feature_id):
        ...
        if self.shape == "rectangle" and not self.maplibre.crs.is_geographic:
            feature_index = self.get_feature_index(feature_id)
...
#        self.set_mode(self.mode)
    ...
    def activate_feature(self, feature_id_or_index):
        ...
        self.maplibre.runjs("./js/draw_layer.js", "activateFeature", arglist=[feature_id])

    def set_feature_geometry(self, feature_id, geom):        
        self.maplibre.runjs("./js/draw_layer.js", "setFeatureGeometry", arglist=[self.map_id, feature_id, geom])
    ...
    def delete_feature(self, feature_id_or_index):
        ...
        if isinstance(feature_id_or_index, int):
            ...
            if len(self.gdf) > 0:
                ...
                self.gdf = gpd.GeoDataFrame()
            # Remove from map
            self.maplibre.runjs("./js/draw_layer.js", "deleteFeature", arglist=[feature_id])
        else:    
            feature_id = feature_id_or_index
            # Remove from gdf
    ...
    def delete_from_map(self):
        ...
        self.active = False
        self.maplibre.runjs("./js/draw_layer.js", "deleteLayer", arglist=[self.map_id])
        self.gdf = gpd.GeoDataFrame()

    def clear(self):
        """Clear the draw layer."""
        for index, row in self.gdf.iterrows():
            self.maplibre.runjs("./js/draw_layer.js", "deleteFeature", arglist=[row["id"]])
        self.gdf = gpd.GeoDataFrame()

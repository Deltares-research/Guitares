...
class CycloneTrackLayer(Layer):
    ...
    def set_data(self,
        ...
        self.maplibre.runjs("/js/line_layer.js", "addLayer", arglist=[id,
                                                                             track_gdf,
                                                                             self.line_color,
                                                                             self.line_width,
                                                                             self.line_opacity,
        ...
        if self.show_icons:
            ...
            self.maplibre.runjs("/js/marker_layer.js", "addLayer", arglist=[id,
    ...
    # def activate(self):
    #     self.maplibre.runjs("./js/geojson_layer_line.js", "setPaintProperties", arglist=[self.map_id,
    #                                                                                    self.line_color,
    #                                                                                    self.line_width,
    #                                                                                    self.line_opacity,
    #                                                                                    self.fill_color,
    ...
    def set_visibility(self, true_or_false):
        ...
        if true_or_false:
            self.maplibre.runjs(self.main_js, "showLayer", arglist=[self.map_id + ".track_line", self.side])
            self.maplibre.runjs(self.main_js, "showLayer", arglist=[self.map_id + ".track_points", self.side])
        else:
            self.maplibre.runjs(self.main_js, "hideLayer", arglist=[self.map_id + ".track_line", self.side])
            self.maplibre.runjs(self.main_js, "hideLayer", arglist=[self.map_id + ".track_points", self.side])

    def delete_from_map(self):
        self.maplibre.runjs(self.main_js, "removeLayer", arglist=[self.map_id + ".track_line", self.side])
        self.maplibre.runjs(self.main_js, "removeLayer", arglist=[self.map_id + ".track_points", self.side])

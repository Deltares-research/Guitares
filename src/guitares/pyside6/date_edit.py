"""PySide6 date-time editor widget wrapper for Guitares GUI elements."""
...
class DateEdit(QDateTimeEdit):
    """Date-time editor that syncs its value with a Guitares variable.

    Parameters
    ----------
    element : Any
        The Guitares element descriptor for this date editor.
    """
    ...
    def set(self) -> None:
        """Update the widget from the linked variable."""
    ...
    def callback(self) -> None:
        """Handle date-time change and fire the element callback."""
        group = self.element.variable_group
        name = self.element.variable
        # newval = self.dateTime().toPyDateTime()
        oldval = self.element.getvar(group, name)
        newval: Union[datetime.datetime, str] = self.dateTime().toPython()
    ...
    def set_geometry(self) -> None:
        """Position and size the date editor and its label."""

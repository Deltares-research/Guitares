...
def update_tipping_points():
    ...
    for loc in locations:
        ...
        visualdelta.gui.map_widget["main_map"].remove_layer(
            layer_name, layer_group_name
        )
        atp_limit = None
        for ii, the in enumerate(loc_df.threshold):
            if visualdelta.slr > the:
                atp_limit = the
        ...
        if atp_limit is not None:
            ...
            marker = Feature(
                geometry=Point((float(loc_atp.x), float(loc_atp.y))),
                properties=properties,
            )
            visualdelta.gui.map_widget["main_map"].add_marker_layer(
    ...
    #                                                             layer_group_name)
    #         properties = {}
    #         loc_atp = loc_df[loc_df.threshold==the]
    #         properties["hover_popup_text"] = loc + ': ' + loc_atp.Description.item()
    # #         properties["hover_popup_text"]  = "<strong>I'm the Oosterscheldekering!</strong><iframe src=https://www.foxnews.com width=500 height=300></iframe>"
    #         properties["hover_popup_width"] = 520
    #

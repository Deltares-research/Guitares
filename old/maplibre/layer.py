...
class Layer:
    def __init__(self, maplibre, id, map_id, **kwargs):
        ...
        if type(self.maplibre).__name__ == "MapLibre":
            # Regular maplibre
            self.main_js = "/js/main.js"
            self.side    = "main"
        elif type(self.maplibre).__name__ == "MapLibreCompare":
            # Compare maplibre
    ...
    def add_layer(self, layer_id, type=None, **kwargs):
        ...
        if type is None:
            ...
            self.layer[layer_id] = Layer(self.maplibre, layer_id, map_id)
            ...
            if type == "circle_selector":
                ...
                self.layer[layer_id] = CircleSelectorLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "polygon":
                from .polygon_layer import PolygonLayer
                self.layer[layer_id] = PolygonLayer(self.maplibre, layer_id, map_id, **kwargs)	
                
            elif type == "polygon_selector":
                from .polygon_selector_layer import PolygonSelectorLayer
                self.layer[layer_id] = PolygonSelectorLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "line_selector":
                from .line_selector_layer import LineSelectorLayer
                self.layer[layer_id] = LineSelectorLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "circle":
                from .circle_layer import CircleLayer
                self.layer[layer_id] = CircleLayer(self.maplibre, layer_id, map_id, **kwargs)
            
            elif type == "line":
                from .line_layer import LineLayer
                self.layer[layer_id] = LineLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "choropleth":
                from .choropleth_layer import ChoroplethLayer
                self.layer[layer_id] = ChoroplethLayer(self.maplibre, layer_id, map_id, **kwargs)
            
            elif type == "heatmap":
                from .heatmap_layer import HeatmapLayer
                self.layer[layer_id] = HeatmapLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "draw":
                from .draw_layer import DrawLayer
                self.layer[layer_id] = DrawLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "image":
                from .image_layer import ImageLayer
                self.layer[layer_id] = ImageLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "raster":
                from .raster_layer import RasterLayer
                self.layer[layer_id] = RasterLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "raster_from_tiles":
                from .raster_from_tiles_layer import RasterFromTilesLayer
                self.layer[layer_id] = RasterFromTilesLayer(self.maplibre, layer_id, map_id, **kwargs)

            # elif type == "deck_geojson":
            #     from .deck_geojson_layer import DeckGeoJSONLayer
            #     self.layer[layer_id] = DeckGeoJSONLayer(self.maplibre, layer_id, map_id, **kwargs)

            # elif type == "datashader_choropleth":
            #     from .datashader_choropleth_layer import DatashaderChoroplethLayer
            #     self.layer[layer_id] = DatashaderChoroplethLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "cyclone_track":
                from .cyclone_track_layer import CycloneTrackLayer
                self.layer[layer_id] = CycloneTrackLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "marker":
                from .marker_layer import MarkerLayer
                self.layer[layer_id] = MarkerLayer(self.maplibre, layer_id, map_id, **kwargs)

            elif type == "raster_tile":
                from .raster_tile_layer import RasterTileLayer
                self.layer[layer_id] = RasterTileLayer(self.maplibre, layer_id, map_id, **kwargs)
    ...
    def delete(self):
        ...
        if self.parent:
            ...
            self.maplibre.layer.pop(self.id)
    ...
    def clear(self):
        ...
        if self.layer:
            ...
            self.maplibre.runjs(self.main_js, "removeLayer", arglist=[self.map_id, self.side])
        ...
        if self.layer:
            ...
            if true_or_false:
                ...
                if self.get_visibility():
                    self.maplibre.runjs(self.main_js, "showLayer", arglist=[self.map_id, self.side])
                else:    
                    self.maplibre.runjs(self.main_js, "hideLayer", arglist=[self.map_id, self.side])
            else:
                self.visible = False
                self.maplibre.runjs(self.main_js, "hideLayer", arglist=[self.map_id, self.side])
    ...
    def set_opacity(self, opacity):
        ...
        self.maplibre.runjs(self.main_js, "setOpacity", arglist=[self.map_id, self.side, opacity])

...
class RasterFromTilesLayer(Layer):
    ...
    def update(self):
        ...
        if self.data is None and self.option == "flood_map":
            return
        overlay_file = os.path.join(self.maplibre.server_path, 'overlays', self.file_name)
        overlay_url  = "./overlays/" + self.file_name
        coords = self.maplibre.map_extent
        xl = [coords[0][0], coords[1][0]]
        yl = [coords[0][1], coords[1][1]]
        wdt = self.maplibre.view.geometry().width()
        hgt = self.maplibre.view.geometry().height()
        ...
        if self.color_values:
            ...
            for file_name in glob.glob(os.path.join(self.maplibre.server_path, "overlays", self.map_id + ".legend.*.png")):
                try:
                    os.remove(file_name)
                except:
                    pass
            ...
            cm2png(self.color_map,
                file_name = os.path.join(self.maplibre.server_path, "overlays", legend_file),
        ...
        if self.new:
            self.maplibre.runjs("/js/image_layer.js", "addLayer", arglist=[overlay_url, self.map_id, bounds, clrbar_dict, self.side])
            # Set legend position (should make this generic for all layers)
            self.maplibre.runjs("/js/image_layer.js", "setLegendPosition", arglist=[self.map_id, self.legend_position, self.side])
        else:
            self.maplibre.runjs("/js/image_layer.js", "updateLayer", arglist=[overlay_url, self.map_id, bounds, clrbar_dict, self.side])
        self.maplibre.runjs("/js/image_layer.js", "setOpacity", arglist=[self.map_id, self.opacity, self.side])
        self.new = False
